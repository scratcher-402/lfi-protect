...
(Local File Inclusion) и File Upload с предотвращением утечек файлов в реальном времени.

## Возможности
- Сканирование URL, форм, JSON запроса на наличие **паттернов Path Traversal** (`../`, `..\`)
- Анализ имён файлов в `multipart/form-data` (**защита от Arbitrary File Upload**)
- **Обнаружение утечек файлов** в ответах веб-сервера
- **Алгоритм Ахо-Корасик** для высокопроизводительного обнаружения утечек
- Удобная **конфигурация в YAML-файле**
- **Блокировка пользователей по IP** при частом срабатывании защиты
- **Просмотр логов** в админ-панели

## Модули
Код программы разделён на несколько модулей:

- `proxy.go` - реализация обратного прокси и анализ запросов
- `files.go` - алгоритм обнаружения утечек и реализация структуры данных (бор) для него
- `logs.go` - логирование
- `admin.go` - админ-панель (backend)
- `ban.go` - блокировка по IP
...
- `proxy.block-enabled` - Включить автоблокировку по IP (true/false)
- `proxy.block-error-rate` - Сколько срабатываний защиты должно произойти за установленный интервал времени, чтобы сработала защита.
- `proxy.block-interval` - интервал времени, за который учитываются срабатывания защиты (в секундах)
#### Обнаружение утечек
...
## Логирование
В проекте реализовано как стандартное для HTTP-серверов логирование (файлы `access.log` и `error.log`), так и файл `info.jsonl`, в котором осуществляется логирование всех модулей приложения в формате newline-separated JSON. 
